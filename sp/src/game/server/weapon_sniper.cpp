...
#define SNIPER_BULLET_COUNT_PLAYER		1	// Fire bullets per shot fired by the player.
#define SNIPER_BULLET_COUNT_NPC			1	// Fire bullets per shot fired by NPCs.
#define SNIPER_TRACER_FREQUENCY_PLAYER	0	// Draw a tracer every shot fired by the player.
#define SNIPER_TRACER_FREQUENCY_NPC		0	// Draw a tracer every shot fired by NPCs.
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponSniper::CapabilitiesGet(void) const
{
...
void CWeaponSniper::PrimaryAttack(void)
{
	...
	if (gpGlobals->curtime >= m_flNextPrimaryAttack)
	{
		...
		// CMachineGun dvs: does this apply to the sniper rifle? I don't know.
		WeaponSound(SINGLE);
...
//-----------------------------------------------------------------------------
const Vector& CWeaponSniper::GetBulletSpread(void)
{
...
void CWeaponSniper::Operator_HandleAnimEvent(animevent_t* pEvent, CBaseCombatCharacter* pOperator)
{
	...
	case EVENT_WEAPON_SNIPER_RIFLE_FIRE:
	{
		...
		{
			AngleVectors(pOperator->GetLocalAngles(), &vecShootOrigin);
